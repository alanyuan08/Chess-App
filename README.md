...
- **NNuE Architecture:** The engine features a customized **Dual-Perspective HalfKA** perspective neural network utilizing a hybrid quantization layout. The architectural data pathways progress as follows:
  ...
  $$\text{Inputs (49,152)} \rightarrow \text{Accumulator (128)} \rightarrow \text{Multiplexed Perspective (128*2)} \rightarrow \text{Hidden 2 (128)} \rightarrow \text{Hidden 3 (32)} \rightarrow \text{Output (1)}$$
  ...
  - **Accumulator Layer:** Shapes into $(49152, 128)$ weights and $(128,)$ biases quantized to signed 16-bit integers (`i16`). Uses branchless tensor multiplexing to concatenate White/Black points of view into a unified $128$-dimensional vector.
