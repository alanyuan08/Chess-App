...
class ChessBoardController(QObject):
	...
	def movePiece(self, moveCommand):
	...
	def listPossibleMovesForPlayer(self, player):
	...
	def returnOpponent(self, player):
	...
	def _possibleMoves(self, row, col, player):
	...
	def totalAttackTargets(self, player):
		attackSquare = {}
		for row in range(0, 8):
			for col in range(0, 8):
				if self.board[row][col] != None and self.board[row][col].player == player:
	...
	def _attackTargets(self, row, col):
		if self.board[row][col] == None:
			return []
		else:
			match self.board[row][col].type:
